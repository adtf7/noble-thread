  ...
  try {
    ...
    if (product.quantity < quantity) {
      return res
        ...
        .json({
          success: false,
          message: "Limited stock, please reduce your quantity.",
        });
    }
    ...
    const discountedPrice =
      largestOffer > 0
        ? product.salePrice * (1 - largestOffer / 100)
        : product.salePrice;
    const qty = parseInt(quantity);
    if (isNaN(qty) || qty < 1 || qty > 5) {
      return res
        .status(400)
        .json({ success: false, message: "Quantity must be between 1 and 5." });
    }
    ...
    if (cart) {
      ...
      if (itemIndex > -1) {
        ...
        if (newTotalQty > 5) {
          return res
            ...
            .json({
              success: false,
              message: "Total quantity cannot exceed 5.",
            });
        }
      ...
      cart = new Cart({
        userId: new mongoose.Types.ObjectId(userId),
        items: [
          { productId, quantity: qty, price: discountedPrice, totalPrice },
        ],
      });
      cart = await cart.save();
    }
...
const loadcheckout = async (req, res) => {
  try {
    ...
    const coupons = allCoupons.filter(
      (coupon) =>
        !userdata.redeemedUser.some(
          (redeemedId) => redeemedId.toString() === coupon._id.toString()
        )
    );
  ...
  try {
    ...
    if (couponId) {
      ...
      if (
        couponData &&
        couponData.isList &&
        subtotal >= couponData.minimumPrice
      ) {
...
const saveFailedOrder = async (userId, orderDetails) => {
  try {
    ...
    console.log("saveFailedOrder received:", {
      userId,
      address,
      orderItems,
      subtotal,
    ...
    if (!requestId)
      throw new Error("Request ID is required for deduplication.");
    ...
    if (existing) {
      console.log("Duplicate failed order detected:", {
      ...
      return {
        success: true,
        message: "Duplicate failed order not saved.",
        orderId: existing._id,
      };
    }
    ...
    return {
      success: false,
      message: error.message || "Failed to save failed order.",
    };
  }
};
...
const createRazorpayOrder = async (req, res) => {
  ...
  try {
    ...
    const { subtotal, discount, taxes, shipping, finalTotal, orderItems } =
      await calculatePriceDetails(userId, couponId);
    if (Math.abs(finalTotal * 100 - amount) > 1) {
      const msg = `Amount mismatch: expected ${finalTotal}, received ${
        amount / 100
      }`;
      console.log("createRazorpayOrder amount mismatch:", {
        finalTotal,
        amount: amount / 100,
      });
...
const handleRazorpayPaymentSuccess = async (req, res) => {
  try {
    ...
    if (
      ...
      console.log("DEBUG saveFailedOrder [res2]", {
        input: {
          address,
          orderItems,
          subtotal: 0,
    ...
    if (generatedSignature !== razorpay_signature) {
      ...
      console.log("DEBUG saveFailedOrder [res3]", {
        input: {
          address,
          orderItems,
          subtotal: 0,
    ...
    if (!cart || !cart.items || cart.items.length === 0) {
      ...
      console.log("DEBUG saveFailedOrder [res4]", {
        input: {
          address,
          orderItems: [],
          subtotal: 0,
      ...
      return res
        .status(400)
        .json({ success: false, message: "Cart is empty.", orderId: null });
    }
    ...
    if (!addressExists) {
      ...
      console.log("DEBUG saveFailedOrder [res5]", {
        input: {
          address,
          orderItems,
          subtotal: 0,
    ...
    if (Math.abs(finalTotal - totalPrice) > 0.01) {
      ...
      console.log("Razorpay Payment: FAILED (Amount mismatch)", {
        finalTotal,
        totalPrice,
      });
      ...
      console.log("DEBUG saveFailedOrder [res6]", {
        input: {
          address,
          orderItems,
          subtotal,
      ...
      return res
        .status(400)
        .json({ success: false, message: msg, orderId: null });
    }
    ...
    for (const item of cart.items) {
      if (!item.productId || !item.quantity || !item.totalPrice) {
        ...
        console.log("DEBUG saveFailedOrder [res7]", {
          input: {
            address,
            orderItems,
            subtotal,
      ...
      if (!product) {
        ...
        console.log("DEBUG saveFailedOrder [res8]", {
          input: {
            address,
            orderItems,
            subtotal,
      ...
      if (isNaN(quantity) || quantity <= 0) {
        ...
        console.log("DEBUG saveFailedOrder [res9]", {
          input: {
            address,
            orderItems,
            subtotal,
    ...
    if (couponId) {
      ...
      if (!couponData || !couponData.isList) {
        ...
        console.log("DEBUG saveFailedOrder [res11]", {
          input: {
            address,
            orderItems,
            subtotal,
        ...
        return res
          ...
          .json({
            success: false,
            message: "Invalid or unlisted coupon.",
            orderId: null,
          });
      }
      await User.findByIdAndUpdate(userId, {
        $push: { redeemedUser: couponId },
      });
    }
    ...
    console.log("DEBUG saveFailedOrder [res12]", {
      input: {
        address: req.body.address || null,
        orderItems,
        subtotal: req.body.subtotal || 0,
...
const placeOrder = async (req, res) => {
  ...
  try {
    ...
    const { subtotal, discount, taxes, shipping, finalTotal, orderItems } =
      await calculatePriceDetails(userId, couponId);
    productsToRestore = orderItems.map((item) => ({
    ...
    if (couponId) {
      ...
      if (!couponData || !couponData.isList) {
        ...
        return res
          .status(400)
          .json({ success: false, message: "Invalid or unlisted coupon." });
      }
      await User.findByIdAndUpdate(userId, {
        $push: { redeemedUser: couponId },
      });
    ...
    if (paymentMethod == "Wallet") {
      ...
      console.log("Order placed successfully (Wallet):", {
        orderId: newOrder._id,
      });
      return res
        .status(200)
        .json({ success: true, order: newOrder, orderId: newOrder._id });
    }
...
const getUpdatedPriceDetails = async (req, res) => {
  try {
    ...
    const { subtotal, discount, taxes, shipping, finalTotal } =
      await calculatePriceDetails(userId, effectiveCouponId);
